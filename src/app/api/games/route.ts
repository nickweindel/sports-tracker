  ...
  try {
    ...
    if (!user_email || !game_date || !home_team || !away_team || !league) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: user_email, league, game_date, home_team, away_team",
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { message: "Game saved successfully", game: data },
      { status: 201 },
    );
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Game deleted successfully", game: data },
      { status: 200 },
    );
  } catch (error) {
