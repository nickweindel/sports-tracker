CREATE OR REPLACE VIEW "public"."vw_games" AS
 SELECT "g"."user_email",
    ...
    "g"."game_center_link",
    COALESCE("ao"."arena", "g"."arena") AS "arena",
    "g"."arena_city",
    "g"."arena_state",
    "g"."arena_country",
    COALESCE("ao"."neutral_site", "g"."neutral_site") AS "neutral_site",
    "g"."notes"
   FROM ((("public"."games" "g"
     ...
     LEFT JOIN "public"."arena_override" "ao" ON ((("g"."game_id" = "ao"."game_id") AND ("g"."league" = "ao"."league")));
